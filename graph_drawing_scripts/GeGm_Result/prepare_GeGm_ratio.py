...
class Cook:
    ...
    def prepare_results(self):
        self.__scans: dict[str, list[tuple[float, tuple[float, float], tuple[float, float], float]]] = dict()
        for elabel, res in self.__results.items():
            ...
            if season not in self.__scans.keys():
                self.__scans[season] = []
            self.__scans[season].append((self.__info[elabel]['mean_energy'], res['Ge2'], res['Gm2'], res['chi2_val'] / res['ndf_val']))
        ...
        for season, vals in self.__scans.items():
            ...
            self.__seasons.append({
                ...
                'chi2' : f'std::vector<double> chi2_{season} = {aux_curly_bracket[0]} {", ".join([str(val) for val in chi2_ndf])} {aux_curly_bracket[1]};'
            })
